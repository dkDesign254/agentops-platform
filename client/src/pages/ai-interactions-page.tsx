        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "var(--space-6)" }}>
          <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
            {loading
              ...
              : data.length === 0
              ? <div style={{ textAlign: "center", padding: "var(--space-16)", color: "var(--color-text-tertiary)", fontFamily: "var(--font-display)" }}>No AI interaction logs yet.</div>
              : data.map((log) => (
                <div key={log.id} style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border-subtle)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
                  <button onClick={() => setExpanded(expanded === log.id ? null : log.id)}
                    ...
                    <span style={{ fontSize: "0.875rem", color: "var(--color-text-primary)", fontFamily: "var(--font-display)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {log.prompt_text ? log.prompt_text.slice(0, 100) + (log.prompt_text.length > 100 ? "..." : "") : "No prompt"}
                    </span>
                    <span style={{ fontSize: "0.75rem", color: "var(--color-text-tertiary)", fontFamily: "var(--font-body)", flexShrink: 0 }}>{log.model_used ?? "n/a"}</span>
                    <span style={{ color: "var(--color-text-tertiary)", fontSize: "0.75rem" }}>{expanded === log.id ? "^" : "v"}</span>
                  </button>
                  {expanded === log.id && (
                    <div style={{ borderTop: "1px solid var(--color-border-subtle)", padding: "var(--space-5)", display: "grid", gap: "var(--space-4)", gridTemplateColumns: "1fr 1fr" }}>
                      <div>
                        <p style={{ fontSize: "0.6875rem", color: "var(--color-text-tertiary)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "var(--space-2)", fontFamily: "var(--font-body)" }}>Prompt</p>
                      ...
                      <div>
                        <p style={{ fontSize: "0.6875rem", color: "var(--color-text-tertiary)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "var(--space-2)", fontFamily: "var(--font-body)" }}>Response</p>
