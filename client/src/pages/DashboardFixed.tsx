...
function RuntimeBadge({ runtime }: { runtime?: string }) {
  ...
  if (value === "make") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full border border-violet-500/20 bg-violet-500/10 px-2.5 py-1 text-[11px] font-medium text-violet-600 dark:text-violet-300">
  ...
  if (value === "n8n") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full border border-orange-500/20 bg-orange-500/10 px-2.5 py-1 text-[11px] font-medium text-orange-600 dark:text-orange-300">
        <Zap className="h-3 w-3" />
        n8n
      </span>
    );
...
function KpiCard({
  ...
  return (
    <button onClick={onClick} className="surface-elevated card-hover rounded-2xl p-5 text-left transition hover:bg-accent/10">
      <div className="mb-4 flex items-center justify-between">
        <div className="rounded-2xl border border-primary/20 bg-primary/10 p-2.5 text-primary">{icon}</div>
      </div>
...
export default function Dashboard() {
  ...
  const { data: workflows = [], isLoading: workflowsLoading, refetch, isFetching } = trpc.airtable.workflows.useQuery(undefined, {
    refetchInterval: 60000,
  });
  ...
  const filteredWorkflows = useMemo(() => {
    ...
    return workflows.filter((workflow) => {
      return (
        ...
        workflow.runtime.toLowerCase().includes(query) ||
      ...
      <div className="mx-auto max-w-[1440px] space-y-6">
        <section className="surface-elevated relative overflow-hidden rounded-3xl p-6 md:p-7">
          ...
          <div className="relative flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div className="space-y-3">
              ...
              <div>
                <h1 className="text-heading text-3xl md:text-4xl">Run and understand company workflows from one place.</h1>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1 text-[11px] font-medium text-emerald-600 dark:text-emerald-300">
                  <span className="h-1.5 w-1.5 rounded-full bg-current" />
                  Workspace live
                </span>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="outline" size="sm" onClick={refresh} disabled={isFetching} className="h-9 rounded-xl border-border bg-background text-xs text-foreground hover:bg-accent disabled:opacity-60">
                <RefreshCw className={`mr-1.5 h-3.5 w-3.5 ${isFetching ? "animate-spin" : ""}`} />
                Refresh
              </Button>
              {canBuild && (
                <Button size="sm" onClick={() => setLocation("/workflows/new")} className="h-9 rounded-xl bg-primary text-xs text-primary-foreground shadow-sm hover:bg-primary/90">
        ...
        <section className="surface-elevated rounded-2xl border border-blue-500/15 bg-blue-500/5 p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">Start with the action you need. Every card below opens the right workspace area directly.</p>
              <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                <button onClick={() => setLocation("/workflows/new")} className="rounded-2xl border border-primary/30 bg-primary/10 p-4 text-left transition hover:bg-primary/15">
                ...
                <button onClick={() => setLocation("/logs")} className="rounded-2xl border border-border/70 bg-background/30 p-4 text-left transition hover:bg-accent/20">
                ...
                <button onClick={() => setLocation("/help")} className="rounded-2xl border border-border/70 bg-background/30 p-4 text-left transition hover:bg-accent/20">
                  <p className="mb-1 text-xs font-semibold text-foreground">3. Ask GAIA AI</p>
                  <p className="text-[11px] leading-relaxed text-muted-foreground">Let the platform guide the next step.</p>
                </button>
              </div>
            </div>
          </div>
        ...
        <section className="grid grid-cols-2 gap-4 xl:grid-cols-4">
          {statsLoading ? (
            ...
            <>
              <KpiCard title="Active Agents" value={stats?.running ?? 0} description="Currently executing" icon={<Bot className="h-4 w-4" />} onClick={() => setLocation("/logs?status=running")} />
              ...
              <KpiCard title="Pending" value={stats?.pending ?? 0} description="Awaiting execution" icon={<Clock className="h-4 w-4" />} onClick={() => setLocation("/logs?status=pending")} />
              <KpiCard title="Errors" value={stats?.failed ?? 0} description="Need investigation" icon={<AlertTriangle className="h-4 w-4" />} onClick={() => setLocation("/logs?status=failed")} />
            </>
          )}
        </section>
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="surface-elevated rounded-2xl p-6">
            <div className="mb-4 flex items-center gap-2">
              <div className="rounded-2xl border border-primary/20 bg-primary/10 p-2 text-primary"><TrendingUp className="h-4 w-4" /></div>
              <div>
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => setLocation("/help")} className="w-full rounded-2xl border border-border/70 bg-background/30 p-4 text-left transition hover:bg-accent/20">
                <p className="text-xs font-semibold text-foreground">Use GAIA AI</p>
        ...
        <section className="space-y-3">
          ...
          <div className="surface-elevated rounded-2xl p-4">
            <div className="relative max-w-sm">
              ...
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search workflows, IDs, runtime, status…" className="h-9 rounded-xl border-border/60 bg-muted/30 pl-9 text-xs" />
            </div>
          </div>
                ...
                <thead>
                  <tr className="border-b border-border bg-muted/15">
                    {["Workflow ID", "Name", "Runtime", "Status", "Requested By", "Requested"].map((label) => (
                      <th key={label} className="whitespace-nowrap px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">{label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {workflowsLoading ? (
                    Array.from({ length: 6 }).map((_, row) => (
                      <tr key={row} className="border-b border-border/40">
                        {Array.from({ length: 6 }).map((__, col) => <td key={col} className="px-4 py-4"><div className="skeleton h-4 rounded-xl" /></td>)}
                      </tr>
                    ))
                  ) : filteredWorkflows.length === 0 ? (
                    <tr><td colSpan={6} className="px-4 py-14 text-center text-sm text-muted-foreground"><Eye className="mx-auto mb-2 h-7 w-7 opacity-30" />No workflows found.</td></tr>
                  ) : (
                    filteredWorkflows.map((workflow, index) => (
                      <tr key={workflow.recordId} onClick={() => setLocation(`/workflows/${workflow.recordId}`)} className={`cursor-pointer border-b border-border/40 transition hover:bg-primary/5 ${index % 2 === 0 ? "bg-card/60" : "bg-background/20"}`}>
        ...
        {role === "viewer" && (
          <section className="surface-elevated rounded-2xl border border-blue-500/15 bg-blue-500/5 p-5">
            <div className="mb-2 flex items-center gap-2"><Eye className="h-3.5 w-3.5 text-blue-500 dark:text-blue-300" /><p className="text-sm font-semibold text-foreground">Read-only mode</p></div>
            <p className="text-xs leading-relaxed text-muted-foreground">Viewer accounts can inspect dashboards, workflow outcomes, reports, and trends, but cannot create or administer workflows.</p>
          </section>
        )}
      </div>
    </DashboardLayout>
