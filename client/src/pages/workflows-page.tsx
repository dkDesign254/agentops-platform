...
export default function WorkflowsPage(): JSX.Element {
  ...
  const rows: WorkflowRow[] = filtered.map((wf) => ({
    id: wf.id, workflowId: wf.workflow_id, workflowName: wf.workflow_name,
    runtime: wf.runtime_used, status: wf.status ?? "Pending",
    reportPeriod: wf.report_period, durationMins: wf.duration_mins,
  }));
  return (
