...
function StatusBadge({ status }: { status: string }) {
  ...
  const cfg: Record<string, { cls: string; dot: string; label: string }> = {
    ...
    running:   { cls: "bg-blue-500/10 text-blue-400 border-blue-500/20", dot: "bg-blue-400", label: "Running" },
    ...
    error:     { cls: "bg-red-500/10 text-red-400 border-red-500/20", dot: "bg-red-400", label: "Error" },
  };
  const c = cfg[s] ?? { cls: "bg-zinc-500/10 text-zinc-400 border-zinc-500/20", dot: "bg-zinc-400", label: status };
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-medium border ${c.cls}`}>
  ...
  return (
    <div className="surface-1 rounded-xl p-5">
      ...
      <div className="h-2 rounded-full overflow-hidden flex gap-0.5 bg-muted/40">
        {stats.completed > 0 && <div className="bg-emerald-500 rounded-full transition-all" style={{ width: pct(stats.completed) }} />}
        {stats.running > 0 && <div className="bg-blue-500 rounded-full transition-all" style={{ width: pct(stats.running) }} />}
        {stats.pending > 0 && <div className="bg-amber-500 rounded-full transition-all" style={{ width: pct(stats.pending) }} />}
        {stats.failed > 0 && <div className="bg-red-500 rounded-full transition-all" style={{ width: pct(stats.failed) }} />}
      </div>
            ...
            <thead>
              <tr className="border-b border-border bg-muted/20">
                {cols.map((col) => (
                  <th
                    key={col.label}
                    className={`text-left px-4 py-2.5 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider whitespace-nowrap ${col.sortable ? "cursor-pointer select-none hover:text-foreground transition-colors" : ""}`}
              ...
              {isLoading ? (
                ...
                filtered.map((wf, idx) => (
                  <tr
                    ...
                    className={`border-b border-border/50 last:border-0 cursor-pointer row-hover ${idx % 2 === 0 ? "bg-card" : "bg-background/40"}`}
                  >
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={() => setLocation("/workflows/new")} className="h-8 text-xs gap-1.5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger-children">
          {statsLoading ? (
            ...
            <>
              <StatCard label="Total Workflows" value={stats?.total ?? 0} icon={<Activity className="w-4 h-4" />} color="default" />
              <StatCard label="Completed" value={stats?.completed ?? 0} icon={<CheckCircle2 className="w-4 h-4" />} color="success" />
              <StatCard label="Pending" value={stats?.pending ?? 0} icon={<Clock className="w-4 h-4" />} color="warning" />
              <StatCard label="Failed" value={stats?.failed ?? 0} icon={<AlertTriangle className="w-4 h-4" />} color="error" />
            </>
          )}
        </div>

        {/* Runtime split + Health + Insights */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="surface-1 rounded-xl p-5 space-y-4">
            ...
            {statsLoading ? (
              ...
              <div className="space-y-3">
                {[
                  { name: "Make", count: stats?.make ?? 0, color: "bg-violet-500", textColor: "text-violet-400", icon: "text-violet-400" },
                  { name: "n8n", count: stats?.n8n ?? 0, color: "bg-orange-500", textColor: "text-orange-400", icon: "text-orange-400" },
                ].map((rt) => {
                  ...
                  return (
                    <div key={rt.name} className="space-y-1.5">
                      ...
                      <div className="h-1.5 bg-muted/50 rounded-full overflow-hidden">
                        <div className={`h-full ${rt.color} rounded-full transition-all duration-500`} style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  );
                })}
