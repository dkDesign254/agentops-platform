...
export function Hero(): JSX.Element {
  ...
  return (
    <section style={{ maxWidth: 1200, margin: "0 auto", padding: "var(--space-20) var(--space-6) var(--space-16)" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-16)", alignItems: "center" }}
        className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        {/* Left — copy */}
        <div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", borderRadius: "var(--radius-full)", border: "1px solid rgba(14,164,114,0.3)", background: "rgba(14,164,114,0.08)", padding: "0.3rem 0.8rem", fontSize: "0.75rem", color: "var(--color-brand)", fontFamily: "var(--font-body)", marginBottom: "var(--space-5)" }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2rem, 5vw, 3.25rem)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.03em", color: "var(--color-text-primary)", marginBottom: "var(--space-5)" }}>
            AI runs your workflows.{" "}
            <span style={{ color: "var(--color-brand)" }}>NexusOps governs them.</span>
          </h1>

          <p style={{ fontSize: "1.0625rem", color: "var(--color-text-secondary)", lineHeight: 1.7, marginBottom: "var(--space-8)", maxWidth: 480 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-3)", marginBottom: "var(--space-8)" }}>
            <button onClick={() => setLocation("/auth?mode=signup")}
              style={{ background: "var(--color-brand)", border: "none", borderRadius: "var(--radius-md)", padding: "0.75rem 1.5rem", fontSize: "0.9375rem", fontWeight: 700, color: "var(--color-text-inverse)", cursor: "pointer", fontFamily: "var(--font-display)", boxShadow: "var(--shadow-brand)", transition: "all var(--transition-base)" }}>
              Start governing free →
            </button>
            <button onClick={() => setDemoOpen(true)}
              style={{ background: "transparent", border: "1px solid var(--color-border-default)", borderRadius: "var(--radius-md)", padding: "0.75rem 1.5rem", fontSize: "0.9375rem", color: "var(--color-text-primary)", cursor: "pointer", fontFamily: "var(--font-display)", transition: "all var(--transition-base)" }}>
              See a live demo
            </button>
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-2)" }}>
              {RUNTIMES.map((rt) => (
                <span key={rt} style={{ fontSize: "0.75rem", padding: "0.25rem 0.6rem", borderRadius: "var(--radius-sm)", background: "var(--color-bg-elevated)", border: "1px solid var(--color-border-subtle)", color: "var(--color-text-secondary)", fontFamily: "var(--font-body)" }}>
                  {rt}
                </span>
              ))}
            </div>
